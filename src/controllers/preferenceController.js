...
function cleanPreference(body, slot) {
  ...
  return {
    ...
    ageRange,
    lifeStage,
    foodLine: species && ['perro', 'gato'].includes(species) ? String(body.foodLine || '').trim() : '',
    medicalCondition: species && ['perro', 'gato'].includes(species) && body.foodLine === 'medicada' ? String(body.medicalCondition || '').trim() : '',
