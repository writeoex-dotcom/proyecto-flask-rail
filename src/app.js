...
function renderDatabaseOfflinePage(req, res, next) {
  ...
  if (req.method === 'POST' && req.path === '/preferences') {
    ...
    fallbackPreferences[preferenceSlot - 1] = {
      ...
      foodLine: species && ['perro', 'gato'].includes(species) ? (req.body.foodLine || '') : '',
      medicalCondition: species && ['perro', 'gato'].includes(species) && req.body.foodLine === 'medicada' ? (req.body.medicalCondition || '') : '',
